...
export function handleImage(context?: MagicColorContext) {
  return ([_, gradient = '', direction, val]: string[], ctx: RuleContext<Theme>) => {
    ...
    if (gradient in linearMap) {
      ...
      for (const dir of linearMap[gradient]) {
        ...
        const result = mcColorResolver(`--un-mask-${dir}-${direction}-color`, hyphenate('colors'), context)([_, val], ctx);
        if (result) {
          const [c, ...p] = result;
          Object.assign(css, c);
          props.push(...p);
      ...
      if (direction == null) {
        ...
        const result = mcColorResolver(`--un-mask-${gradient}-${direction}-color`, hyphenate('colors'), context)([_, val], ctx);
        if (result) {
          const [c, ...p] = result;
          Object.assign(css, c);
          props.push(...p);
